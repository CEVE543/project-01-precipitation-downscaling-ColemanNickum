...
function knn_quantile_mapping(temp_train, temp_test, precip_train, precip_test; n_pca::Int, n_trees::Int)
    ...
    for i in 1:length(y_pred_corrected)
        if !ismissing(y_pred_corrected[i])
            ...
            # Find the quantile of the actual value in the observed distribution
...
```{julia}
"""
temp_train: the training temperature data. This should be the raw field, not the principal components.
temp_test: the temperature data to predict. This should be the raw field, not the principal components.
precip_train: the training precipitation data.
"""
function predict_svm(temp_train, temp_test, precip_train; n_pca::Int)
    ...
    # build the SVM model
    svm_model = LIBSVM.svmtrain(train_embedded, precip_labels)

    # apply the learned model to the test data
    predicted_labels, decision_values = LIBSVM.svmpredict(svm_model, test_embedded)

    # reshape the predictions to match the original shape of precip_train
    precip_pred = reshape(predicted_labels, size(precip_train))
...
precip_pred_svm = predict_svm(temp_train, temp_test, precip_train; n_pca=n_pca)

# Now, `precip_pred_svm` contains the predicted precipitation values.
