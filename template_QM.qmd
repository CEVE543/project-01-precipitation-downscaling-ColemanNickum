...
# Random Forest
```{julia}
transformed_data = PCA_model.transform(PCA_model)
combined_data = hcat(transformed_data, precip_train)
using DecisionTree

# Assuming combined_data is your combined dataset
features = combined_data[:, 1:end-1]
target = combined_data[:, end]

model = DecisionTreeClassifier()  # or DecisionTreeRegressor() for regression
fit!(model, features, target)

new_data = transform(precip_test, PCA_model)
predictions = predict(model, precip_test)
