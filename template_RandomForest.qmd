...
```{julia}
y_train = convert(Array{Float32}, coalesce.(precip_train, 0.0))
y_test = convert(Array{Float32}, coalesce.(precip_test, 0.0))
```


```{julia}
# Reshape back to (lon, lat, time)
function train_random_forest(temp_train, temp_test, precip_train; n_pca::Int, n_trees)
