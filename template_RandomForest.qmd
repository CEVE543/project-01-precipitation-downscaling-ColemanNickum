...
```{julia}
    pc = projection(PCA_model)[:, 1];
```

# Random Forest

```{julia}
X_train_pca = convert(Array{Float32}, X_train_pca)
X_test_pca = convert(Array{Float32}, X_test_pca)
```
...
```{julia}
    X_train_pca = MultivariateStats.transform(PCA_model, X_train_pca)
    X_test_pca = MultivariateStats.transform(PCA_model, X_test_pca)
```
```{julia}
X_train_pca = reshape(temp_train, 21 * 33, 11688)
X_test_pca = reshape(temp_test, 21 * 33, 3653)
```
```{julia}
...
```{julia}
    y_train = reshape(precip_train,10 * 10, 11688)
    y_test = reshape(precip_test, 10 * 10, 3653)
```

```{julia}
# Reshape back to (lon, lat, time)
function train_random_forest(temp_train, precip_train, temp_test, precip_test, n_components, n_trees)

    # Assuming temp_train, precip_train, temp_test, precip_test are already defined

    # Reshape the temperature data for PCA
    X_train_pca = preprocess(temp_train, temp_train);
    X_test_pca = preprocess(temp_test, temp_train);
   
   # Apply PCA transformation
    PCA_model = fit(PCA, X_train_pca, maxoutdim=n_components)

    # Reshape back to (lon, lat, time)
    X_train_pca = convert(Array{Float32}, X_train_pca)
    X_test_pca = convert(Array{Float32}, X_test_pca)

    # Flatten precipitation data to use as target
    y_train = convert(Array{Float32}, (precip_train)) 
    y_test = convert(Array{Float32}, (precip_test)) 
    ...
    rf_model = DecisionTree.build_forest(y_train, X_train_pca, n_trees)

    # Make predictions on the test set
    y_pred = DecisionTree.apply_forest(rf_model, X_test_pca)
