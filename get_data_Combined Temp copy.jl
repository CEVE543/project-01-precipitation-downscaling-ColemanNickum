...
function run_demo()
    ...
    years = 1972:2022 # example time range
    for year in years
    ...
    # Create a new NetCDF file to save all the data
    output_file = joinpath(HOMEDIR, "data", "new_data.nc")
    ds = Dataset(output_file, "c")
    
    # Create dimensions for time and lat/lon (adjust dimensions as needed)
    define(ds, "time", size(t2m["time"])[1])
    define(ds, "lat", size(t2m["latitude"])[1])
    define(ds, "lon", size(t2m["longitude"])[1])

    # Create variables
    var_t2m = defVar(ds, "t2m", Float32, ("time", "lat", "lon"))

    # Set the attributes for variables
    setncatts(var_t2m, Dict("units" => "Kelvin"))

    # Write the data to the NetCDF file
    write(ds, "time", t2m["time"][:])
    write(ds, "lat", t2m["latitude"][:])
    write(ds, "lon", t2m["longitude"][:])
    write(ds, "t2m", t2m["t2m"][:])

    close(ds) # Close the NetCDF file
